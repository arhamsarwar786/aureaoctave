...
export default function DepositForm({ btc_address }) {
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
            ...
            <div className="flex flex-col items-center lg:items-start gap-6">
                <div className="bg-[#111A22] p-6 rounded-2xl border border-slate-700 shadow-lg">
                    <img src={QRCode} alt="Deposit QR Code" className="w-56 h-56 object-contain" />
                </div>
                {/* BTC Address Box */}
                <div className="w-full">
                    <label className="text-sm text-slate-400">Bitcoin Address</label>
                    <div className="mt-2 flex items-center bg-[#0B141B] border border-slate-700 rounded-xl overflow-hidden">
                        <input type="text" value={btc_address} disabled className="flex-1 bg-transparent text-slate-300 px-4 py-3 text-sm focus:outline-none" />
                        <CopyToClipboard text={btc_address} onCopy={handleCopy}>
                            <button type="button" className="px-4 py-3 bg-[#3BF5C4]/10 text-[#3BF5C4] hover:bg-[#3BF5C4]/20 transition font-medium">
            ...
            <form onSubmit={onSubmit} className="space-y-8">
                ...
                <div>
                    ...
                    <input type="text" value={data.pasted_transaction_id} onChange={e => setData("pasted_transaction_id", e.target.value)} required className="mt-2 w-full bg-[#0B141B] border border-slate-700 text-white rounded-xl px-4 py-3 focus:border-[#3BF5C4] focus:ring-0 transition" placeholder="Paste transaction hash" />
                    <InputError message={errors.pasted_transaction_id} className="mt-2" />
                </div>
